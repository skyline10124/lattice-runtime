...
use lattice::agent::Agent;
use lattice::core::catalog::{ApiProtocol, CredentialStatus};
use lattice::core::ResolvedModel;
...
fn test_agent_send_message_basic() {
    ...
    let tokens: String = events
        ...
        .filter_map(|e| match e {
            lattice::agent::LoopEvent::Token { text } => Some(text.as_str()),
            _ => None,
        })
        .collect();
    ...
    let has_done = events
        .iter()
        .any(|e| matches!(e, lattice::agent::LoopEvent::Done { .. }));
...
fn test_agent_run_streaming_observer_receives_tokens() {
    ...
    let events = rt.block_on(agent.run_streaming("Test stream", 3, |event| {
        if let lattice::agent::LoopEvent::Token { text } = event {
...
fn test_agent_run_with_tool_call_no_executor() {
    ...
    let has_tool_call = events
        .iter()
        .any(|e| matches!(e, lattice::agent::LoopEvent::ToolCallRequired { .. }));
    assert!(
        has_tool_call,
