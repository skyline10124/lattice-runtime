use crate::agent::memory::{EntryKind, Memory, MemoryEntry};
...
impl<P: Plugin + ?Sized> ErasedPlugin for ErasedPluginAdapter<'_, P> {
    ...
    fn tools(&self) -> &[crate::core::types::ToolDefinition] {
        Plugin::tools(self.0)
    }
    fn preferred_model(&self) -> &str {
...
impl<'a, P: Plugin + ?Sized, B: Behavior, A: PluginAgent> PluginRunner<'a, P, B, A> {
    ...
    pub async fn run(&mut self, input: &P::Input) -> Result<RunResult, PluginError> {
        ...
        crate::plugin::erased_runner::run_plugin_loop(
            &adapter,
            self.behavior,
            self.agent,
            &context,
...
pub use crate::agent::PluginAgent;
...
fn timestamp() -> String {
    crate::core::memory::now_ms().to_string()
}
...
mod tests {
    ...
    fn test_behavior_mode_to_behavior() {
        use crate::plugin::bundle::{behavior_to_behavior_trait, BehaviorMode, YoloSandboxPolicy};
        ...
        assert!(matches!(behavior.decide(0.9), crate::plugin::Action::Done));
        assert!(matches!(behavior.decide(0.5), crate::plugin::Action::Retry));
        assert!(matches!(
            behavior.on_error(&crate::plugin::PluginError::Parse("x".into()), 3),
        ...
        assert!(matches!(behavior.decide(0.1), crate::plugin::Action::Done));
    }

    #[test]
    fn test_plugin_registry_register_and_get() {
        use crate::plugin::bundle::{BehaviorMode, PluginBundle, PluginMeta, YoloSandboxPolicy};
