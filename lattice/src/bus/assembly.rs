...
use crate::agent::{default_tool_definitions, memory::Memory, Agent, DefaultToolExecutor};
use tracing::warn;

use crate::agent::tool_registry::ToolRegistry;
use crate::bus::profile::AgentProfile;
use crate::bus::runner::AgentRunner;
...
pub fn assemble_agent(
    ...
    resolved: crate::core::ResolvedModel,
    ...
    let (final_tools, executor) = if let Some(reg) = &tool_registry {
        if !reg.is_empty() {
            let registry_defs: Vec<crate::core::types::ToolDefinition> = reg
                .definitions()
                .into_iter()
            ...
            let exec = DefaultToolExecutor::new_with_registry(
                ".",
                Some(reg.clone() as Arc<dyn crate::agent::RegistryToolAccess>),
            )
