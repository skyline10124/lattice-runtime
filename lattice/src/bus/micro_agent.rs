...
use crate::agent::memory::SharedMemory;
use crate::agent::memory::{Memory, MemoryEntry, PartitionAccess, SharedPartition};
...
use crate::bus::profile::{AgentProfile, BusConfigProfile, MemoryConfigProfile};
...
impl MicroAgent {
    ...
    pub async fn spawn(self) -> Result<MicroAgentHandle, BusError> {
        ...
        let resolved = if let Some(router) = &self.model_router {
            router.resolve(&self.profile.agent.model, None)
        } else {
            let router = match &self.credentials {
                Some(creds) => crate::core::router::ModelRouter::with_credentials(creds.clone()),
                None => crate::core::router::ModelRouter::new(),
            };
            router.resolve(&self.profile.agent.model, None)
        };
        let resolved = resolved.map_err(|e| BusError::Resolve(e.to_string()))?;

        let runner = crate::bus::assembly::assemble_agent(
            &self.profile,
            resolved,
            self.memory.clone(),
...
async fn micro_agent_loop(
    mut runner: crate::bus::runner::AgentRunner,
...
mod tests {
    ...
    fn extract_content(events: &[crate::agent::LoopEvent]) -> String {
    ...
    fn parse_output(content: &str) -> serde_json::Value {
        crate::bus::json_output::parse_json_or_content(content)
    }
