...
use crate::bus::events::{EventBus, PipelineEvent};
use crate::bus::handoff_rule::eval_rules;
use crate::bus::profile::AgentProfile;
use crate::bus::registry::AgentRegistry;
use crate::core::handoff::HandoffTarget;
use crate::plugin::dag_runner::PluginDagRunner;
...
pub struct Pipeline {
    ...
    /// Shared model router supplied by the top-level runtime.
...
impl Pipeline {
    ...
    pub fn with_plugin_registry(
        mut self,
        pr: Arc<crate::plugin::registry::PluginRegistry>,
    ) -> Self {
    ...
    pub async fn run(&mut self, start_agent: &str, input: &str) -> PipelineRun {
        ...
        for _turn in 0..self.max_pipeline_iterations {
            ...
            let resolved = match self.resolve_model(&profile.agent.model) {
                Ok(r) => r,
                Err(e) => {
            ...
            let output_result: Result<serde_json::Value, String> =
                if let (Some(plugins_config), Some(pr)) = (&profile.plugins, &self.plugin_registry)
                {
                    ...
                    let mut dag_runner = PluginDagRunner::new(
                        ...
                        pr,
                        tr_ref,
                        crate::core::retry::RetryPolicy::default(),
                    ...
                    if let Some(router) = &self.model_router {
                        dag_runner = dag_runner.with_model_router(Arc::clone(router));
                    }
                    ...
                    match crate::bus::assembly::assemble_agent(
                        &profile,
                        resolved,
                        self.shared_memory.clone(),
    ...
    fn resolve_model(
        &self,
        model: &str,
    ) -> Result<crate::core::ResolvedModel, crate::core::LatticeError> {
        if let Some(router) = &self.model_router {
            return router.resolve(model, None);
        }
        match &self.credentials {
            Some(creds) => ModelRouter::with_credentials(creds.clone()).resolve(model, None),
            None => ModelRouter::new().resolve(model, None),
        }
    }
            ...
            .map(|agent_name| {
                ...
                tokio::spawn(async move {
                    let branch_future = async {
                        ...
                        let resolved = match Self::resolve_model_with(
                            model_router.as_ref(),
                            credentials.as_ref(),
                            &profile.agent.model,
                        ) {
                        ...
                        let mut runner = match crate::bus::assembly::assemble_agent(
                            &profile,
                            resolved,
                            memory_box.clone(),
                            tool_registry.clone(),
    ...
    fn save_memory_entry(&self, profile: &AgentProfile, output: &serde_json::Value) {
        if let Some(ref mem) = self.shared_memory {
            ...
            let ms = crate::agent::memory::now_ms();
            let entry = crate::agent::memory::MemoryEntry {
                id: format!("{}-{}", profile.agent.name, ms),
                kind: crate::agent::memory::EntryKind::SessionLog,
