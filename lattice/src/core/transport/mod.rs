...
use crate::core::streaming::{SseParser, StreamEvent};
...
pub fn chat_response_to_stream(
    response: crate::core::provider::ChatResponse,
) -> Vec<crate::core::streaming::StreamEvent> {
    let mut events = Vec::new();

    if let Some(ref content) = response.content {
        if !content.is_empty() {
            events.push(crate::core::streaming::StreamEvent::Token {
    ...
    if let Some(ref tool_calls) = response.tool_calls {
        for tc in tool_calls {
            events.push(crate::core::streaming::StreamEvent::ToolCallStart {
                id: tc.id.clone(),
                name: tc.function.name.clone(),
            });
            events.push(crate::core::streaming::StreamEvent::ToolCallDelta {
                id: tc.id.clone(),
                arguments_delta: tc.function.arguments.clone(),
            });
            events.push(crate::core::streaming::StreamEvent::ToolCallEnd { id: tc.id.clone() });
        }
    }

    events.push(crate::core::streaming::StreamEvent::Done {
...
pub trait Transport: Send + Sync {
    ...
    ///
    /// Default: [`crate::core::streaming::OpenAiSseParser`].
    fn create_sse_parser(&self) -> Box<dyn SseParser> {
        Box::new(crate::core::streaming::OpenAiSseParser::new())
    }
