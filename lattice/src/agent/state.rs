...
use crate::agent::message_source::{MessageSource, SourceRegistry};
...
impl AgentState {
    ...
    pub fn push_assistant_message(
        ...
        content: &str,
        reasoning: &str,
        tool_calls: Option<Vec<crate::core::types::ToolCall>>,
    ) {
    ...
    pub fn trim_messages(&mut self, context_length: u32, safety_margin_percent: u8) {
        ...
        let system_msgs: Vec<_> = self
            .messages
            .iter()
            .filter(|m| matches!(m.role, crate::core::Role::System))
            .cloned()
            .collect();
        ...
        let estimate_msg = |msg: &crate::core::types::Message| -> u32 {
            crate::core::tokens::TokenEstimator::estimate_text_for_model(
                &msg.content,
        ...
        while non_system
            .first()
            .is_some_and(|m| m.role == crate::core::Role::Tool)
        {
        ...
        while non_system.len() > 2 {
            ...
            if removed.role == crate::core::Role::Assistant && removed.tool_calls.is_some() {
                while non_system
                    .first()
                    .is_some_and(|m| m.role == crate::core::Role::Tool)
                {
...
mod tests {
    ...
    fn make_resolved(context_length: u32) -> ResolvedModel {
        ResolvedModel {
            ...
            credential_status: crate::core::CredentialStatus::Missing,
        }
    }
