...
fn catalog_loads_successfully_with_embedded_data() {
    ...
    let catalog =
        lattice_core::catalog::Catalog::get().expect("catalog must load from embedded data.json");
    assert!(
...
fn catalog_claude_sonnet_entry_exists() {
    let catalog = lattice_core::catalog::Catalog::get().expect("catalog must load");
    let model = catalog
...
fn error_classify_is_retryable_rate_limit() {
    ...
    assert!(
        lattice_core::errors::ErrorClassifier::is_retryable(&err),
