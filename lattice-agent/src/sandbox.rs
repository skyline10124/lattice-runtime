...
impl SandboxConfig {
    ...
    pub fn check_command(&self, cmd: &str) -> Result<(), String> {
            ...
            match chain.run_pre_execute("bash", cmd) {
                ...
                Err(reason) => {
                    ...
                    if let Some(ref audit) = self.audit_log {
                        audit.log_sync(crate::audit::AuditEvent::hook_deny(
                            &reason.hook,
                            "bash",
                            cmd,
                            &reason.reason,
        ...
        if let Some(ref audit) = self.audit_log {
            ...
            audit.log_sync(crate::audit::AuditEvent::tool_exec(
                "bash",
                &cmd,
                &self.sandbox_label,
                outcome,
    ...
    pub fn check_url(&self, url_str: &str) -> Result<(), String> {
        ...
        if scheme == "https" {
            if lattice_core::security::is_private_ip(host) {
                return Err(format!(
...
mod tests {
    ...
    fn test_is_private_ip() {
        use lattice_core::security::is_private_ip;
    ...
    fn vet_sandbox() -> SandboxConfig {
        ...
        config.hook_chain = Some(std::sync::Arc::new(HookChain::new(vec![
            Box::new(crate::hook::VetHook::default()),
            Box::new(crate::hook::TirithHook::new()),
        ])));
        config
    }
    ...
    fn security_pipeline_vet_intercepts_curl_download() {
        let vet = crate::hook::VetHook::default();
        let tirith = crate::hook::TirithHook::new();
    ...
    async fn sandbox_config_audit_logs_hook_denials() {
        ...
        let audit = std::sync::Arc::new(crate::audit::AuditLog::new(dir.clone()));
        ...
        audit
            .log(crate::audit::AuditEvent::tool_exec(
                "bash",
                "rm -rf /",
                "permissive",
                "denied",
